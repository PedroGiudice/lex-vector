"""Funcoes auxiliares para o frontend."""


def get_icon(name: str, color: str = "#e0e0e0", size: int = 16) -> str:
    """
    Retorna SVG string para icone.

    Args:
        name: Nome do icone (bot, folder, git, file, zap, spinner, play, wrench)
        color: Cor do stroke em hex
        size: Tamanho em pixels

    Returns:
        String SVG para uso em HTML
    """
    icons = {
        ...
        "spinner": f'<svg class="custom-spinner" xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>',
        ...
        "stop": f'<svg xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg>',
...
def truncate_path(path: str, max_length: int = 40) -> str:
    """Trunca path longo para exibicao."""
