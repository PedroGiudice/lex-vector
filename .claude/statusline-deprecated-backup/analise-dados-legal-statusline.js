#!/usr/bin/env node
/**
 * analise-dados-legal-statusline.js - Status line para Análise de Dados Legais
 *
 * Exibe em tempo real:
...
async function main() {
  try {
    ...
    console.log(`${colors.yellow}Analise-Dados-Legal Status${colors.reset} ${colors.dim}(error loading data)${colors.reset}`);
  }
}
...
function generateHeader(model, workspace, git, cost, tokens) {
  ...
  return `${colors.bold}${colors.cyan}[ANALISE-DADOS-LEGAL]${colors.reset} ` +
