#!/usr/bin/env node
/**
 * qualidade-codigo-statusline.js - Status line para Qualidade de Código
 *
 * Exibe em tempo real:
...
async function main() {
  try {
    ...
    console.log(`${colors.yellow}Qualidade-Codigo Status${colors.reset} ${colors.dim}(error loading data)${colors.reset}`);
  }
}
...
function generateHeader(model, workspace, git, cost, tokens) {
  ...
  return `${colors.bold}${colors.cyan}[QUALIDADE-CODIGO]${colors.reset} ` +
