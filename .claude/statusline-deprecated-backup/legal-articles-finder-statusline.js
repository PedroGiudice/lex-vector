#!/usr/bin/env node
/**
 * legal-articles-finder-statusline.js - Status line para Legal Articles Finder
 *
 * Exibe em tempo real:
...
async function main() {
  try {
    ...
    console.log(`${colors.yellow}Legal-Articles-Finder Status${colors.reset} ${colors.dim}(error loading data)${colors.reset}`);
  }
}
...
function generateHeader(model, workspace, git, cost, tokens) {
  ...
  return `${colors.bold}${colors.cyan}[LEGAL-ARTICLES-FINDER]${colors.reset} ` +
