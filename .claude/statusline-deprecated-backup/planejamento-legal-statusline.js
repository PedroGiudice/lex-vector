#!/usr/bin/env node
/**
 * planejamento-legal-statusline.js - Status line para Planejamento Legal
 *
 * Exibe em tempo real:
...
async function main() {
  try {
    ...
    console.log(`${colors.yellow}Planejamento-Legal Status${colors.reset} ${colors.dim}(error loading data)${colors.reset}`);
  }
}
...
function generateHeader(model, workspace, git, cost, tokens) {
  ...
  return `${colors.bold}${colors.cyan}[PLANEJAMENTO-LEGAL]${colors.reset} ` +
