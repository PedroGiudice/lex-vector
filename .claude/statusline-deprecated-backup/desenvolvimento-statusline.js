#!/usr/bin/env node
/**
 * desenvolvimento-statusline.js - Status line para Desenvolvimento
 *
 * Exibe em tempo real:
...
async function main() {
  try {
    ...
    console.log(`${colors.yellow}Desenvolvimento Status${colors.reset} ${colors.dim}(error loading data)${colors.reset}`);
  }
}
...
function generateHeader(model, workspace, git, cost, tokens) {
  ...
  return `${colors.bold}${colors.cyan}[DESENVOLVIMENTO]${colors.reset} ` +
