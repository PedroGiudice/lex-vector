...
Our platform API which is powered by marker and surya and is easy to test out - it's free to sign up, and we'll include credits, try it out here
Our painless on-prem solution for commercial use, which you can read about here and gives you privacy guarantees with high throughput inference optimizations.
Deployment example with Modal that shows you how to deploy and access marker through a web endpoint using Modal. Modal is an AI compute platform that enables developers to deploy and scale models on GPUs in minutes.