...
import useTrelloStore from '@/store/trelloStore'; // Import the new Trello store

// Import the new panel components (will create these next)
...
export function MainLayout({ children: _children }: MainLayoutProps) {
  ...
  const syncStatusColor = isLoading
    ? 'bg-status-yellow'
    : error
      ? 'bg-status-red'
      : 'bg-status-emerald';
  ...
  const timeSinceLastSync = lastSync
    ? Math.floor((new Date().getTime() - lastSync.getTime()) / 1000)
    : null;

  return (
    <div className="flex flex-col h-screen w-screen bg-bg-main text-text-primary">
      ...
      <header className="flex items-center justify-between px-4 py-2 border-b border-border-default bg-bg-panel-1 h-12">
        <div className="flex items-center space-x-3">
          <span className="text-accent-indigo font-bold text-base flex items-center gap-2">
            ...
            >
              ...
              <path
                d="M5.63604 5.63604L7.05025 7.05025M16.9497 16.9497L18.364 18.364"
                strokeLinecap="round"
              />
              <path
                d="M5.63604 18.364L7.05025 16.9497M16.9497 7.05025L18.364 5.63604"
                strokeLinecap="round"
              />
        ...
        <div className="flex items-center space-x-3">
          ...
          <span className="flex items-center text-text-muted text-xxs font-mono">
            <span className={`w-2 h-2 rounded-full ${syncStatusColor} mr-1.5`}></span>
            {syncStatusText}{' '}
            {timeSinceLastSync !== null && !isLoading && !error ? `${timeSinceLastSync}s ago` : ''}
          </span>
          <button
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Navigation Panel (Left - 20%) */}
        <NavigationPanel />

        {/* Data Table Panel (Center - 55%) */}
        <DataTable />

        {/* Actions Panel (Right - 25%) */}
        <ActionsPanel />
      </div>
