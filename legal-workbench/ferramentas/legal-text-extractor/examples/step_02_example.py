...
def example_performance_optimized():
    ...
    for doc_id, layout_path, pdf_path in documents:
        try:
            results = processor.process(Path(layout_path), Path(pdf_path), doc_id)
...
def example_error_handling():
    ...
    try:
        processor.process(
            Path("outputs/missing/layout.json"), Path("test-documents/dummy.pdf"), "test"
        )
    ...
    print("\nTeste 2: PDF inexistente")
    try:
        processor.process(Path("outputs/exemplo_scan/layout.json"), Path("missing.pdf"), "test")
    except FileNotFoundError as e:
        print(f"  ✓ Erro capturado: {e}")
