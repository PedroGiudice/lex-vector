...
class CleanerEngine:
    ...
    def _setup_profiles(self):
        ...
        self.profiles["PJE"] = SystemProfile(
            ...
            cleaning_rules=[
                ...
                re.compile(
                    r"este\s+documento\s+foi\s+gerado\s+pelo\s+usu[áa]rio.*?\d{2}:\d{2}:\d{2}",
                    re.I | re.DOTALL,
                ),
                re.compile(
                    r"documento\s+assinado\s+por\s+[^\n]{5,100}\s+e\s+certificado\s+digitalmente",
                    re.I,
                ),
                re.compile(
                    r"^[_\-=]+\s*processo\s+judicial\s+eletr[ôo]nico\s*[_\-=]+$", re.I | re.M
                ),
            ],
            weight=1.5,
        )
        ...
        self.profiles["ESAJ"] = SystemProfile(
            ...
            cleaning_rules=[
                ...
                re.compile(
                    r"confer[êe]ncia\s+de\s+documento\s+digital.*?portal\s+e-saj", re.I | re.DOTALL
                ),
                re.compile(
                    r"assinado\s+digitalmente\s+por:\s*[^\n]{5,80}\s+data:\s*\d{2}/\d{2}/\d{4}",
                    re.I,
                ),
        ...
        self.profiles["PROJUDI"] = SystemProfile(
            ...
            cleaning_rules=[
                re.compile(
                    r"digitalmente\s+assinado\s+por.*?data:\s*\d{2}/\d{2}/\d{4}", re.I | re.DOTALL
                ),
    ...
    def _setup_universal(self):
        ...
        self.universal_patterns = [
            ...
            re.compile(
                r"^Da\s*t[aL]/H[oO]r[aA][:\s,]*$", re.I | re.M
            ),  # corrupted "Data/Hora" from OCR
    ...
    def detect_system(self, text: str) -> DetectionResult:
        ...
        if not scores or max(scores.values()) == 0:
            return DetectionResult(
                system="UNKNOWN", confidence=0.0, hits=0, total_fingerprints=total_fingerprints
            )
    ...
    def clean(self, text: str, force_system: str | None = None) -> str:
        ...
        if force_system and force_system in self.profiles:
            detection = DetectionResult(
                system=force_system, confidence=1.0, hits=0, total_fingerprints=0
            )
        else:
