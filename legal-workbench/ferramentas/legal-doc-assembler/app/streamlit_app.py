...
with st.sidebar:
    ...
    st.markdown('<span class="text-muted">Workbench v2.1</span>', unsafe_allow_html=True)

    st.markdown("---")

    nav_mode = st.radio(
        "Navigation",
        ["Data Input", "Batch Processing", "Template Builder", "Assembler", "Source Code"],
...
# -----------------------------------------------------------------------------
# DATA INPUT PAGE (SINGLE)
# -----------------------------------------------------------------------------

if nav_mode == "Data Input":
    st.markdown("## Data Input (Single Document)")
...
# -----------------------------------------------------------------------------
...
elif nav_mode == "Batch Processing":
    st.markdown("## Batch Processing")
    st.markdown('<span class="text-muted">Process multiple documents at once (High-Performance).</span>', unsafe_allow_html=True)

    # 1. Upload Section
    st.markdown("#### 1. Input Data")
    
    upload_type = st.radio("Upload Method", ["Multiple JSON Files", "ZIP Archive", "Directory (Server-side)"], horizontal=True)
    
    loaded_docs = []

    if upload_type == "Multiple JSON Files":
        uploaded_files = st.file_uploader("Upload JSON files", type="json", accept_multiple_files=True)
        if uploaded_files:
            for f in uploaded_files:
                try:
                    data = json.load(f)
                    # Add filename meta if not present
                    if 'meta_filename' not in data:
                        data['meta_filename'] = f.name
    ...
    elif upload_type == "Directory (Server-side)":
        dir_path = st.text_input("Server Directory Path", value="/home/cmr-auto/claude-work/repos/Claude-Code-Projetos/legal-workbench/ferramentas/trello-mcp/output/cards") # Updated default path
    ...
    if loaded_docs:
        st.session_state.batch_data = loaded_docs
        st.success(f"Loaded {len(loaded_docs)} documents for batch processing")
    
    # 2. Data Preview (Clean)
    if st.session_state.batch_data:
        st.markdown("#### 2. Data Preview")
        
        # Create small dataframe for preview
        preview_data = []
        for d in st.session_state.batch_data[:50]: # Limit to 50 for preview
            row = {
                'Filename': d.get('meta_filename', 'N/A'),
                'Name': d.get('nome') or d.get('name') or d.get('description_data', {}).get('name', 'N/A'),
                'ID/CPF': d.get('cpf') or d.get('cnpj') or d.get('description_data', {}).get('cpf', 'N/A'),
                'Status': 'Ready'
            }
        ...
        if len(st.session_state.batch_data) > 50:
            st.info(f"Showing first 50 of {len(st.session_state.batch_data)} documents")

    # 3. Action
    st.markdown("#### 3. Execution")
    
    col_batch_btn, col_batch_info = st.columns([1, 2])
    
    with col_batch_btn:
        can_run = len(st.session_state.batch_data) > 0 and st.session_state.template_content is not None
        
        if st.button("RUN BATCH PROCESSOR", type="primary", disabled=not can_run, use_container_width=True):
            if not st.session_state.template_content:
                st.error("Please load a template in 'Template Builder' tab first!")
            else:
                progress_bar = st.progress(0)
                status_text = st.empty()
                
                # Setup batch processor
                # We need to save the json data to temp files because BatchProcessor works with paths
                # This is an adapter step since we loaded into memory
                
                temp_input_dir = Path(tempfile.mkdtemp(prefix="legal_batch_input_"))
                temp_output_dir = Path(tempfile.mkdtemp(prefix="legal_batch_output_"))
                
                status_text.text("Preparing temporary files...")
                
                json_files_paths = []
                for i, doc in enumerate(st.session_state.batch_data):
                    fname = doc.get('meta_filename', f'doc_{i}.json')
                    fpath = temp_input_dir / fname
                    with open(fpath, 'w', encoding='utf-8') as f:
                        json.dump(doc, f, ensure_ascii=False)
                    json_files_paths.append(fpath)
                
                status_text.text(f"Processing {len(json_files_paths)} documents...")
                ...
                try:
                    results = processor.process_batch(
                        json_files=json_files_paths,
                        template_path=Path(st.session_state.template_content),
                        output_dir=temp_output_dir,
                        create_zip=True
                    )
                    
                    st.session_state.batch_results = results
                    st.session_state.batch_results['temp_dir'] = str(temp_output_dir) # Save for download
    ...
    with col_batch_info:
        if not can_run:
            if not len(st.session_state.batch_data) > 0:
                st.warning("Waiting for data...")
            elif st.session_state.template_content is None:
                st.warning("Waiting for template (Load in 'Template Builder' tab)")

    # 4. Results
    if st.session_state.batch_results:
        st.markdown("---")
        st.markdown("#### 4. Results Download")
        
        res = st.session_state.batch_results
        
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total", res['total'])
        c2.metric("Success", res['success'])
        c3.metric("Failed", res['failed'])
        c4.metric("Time", f"{res['duration_seconds']}s")
        
        if 'zip_path' in res and Path(res['zip_path']).exists():
            with open(res['zip_path'], "rb") as f:
                st.download_button(
                    label="📥 Download All (ZIP)",
                    data=f.read(),
                    file_name=Path(res['zip_path']).name,
        ...
        if res['failed'] > 0:
            with st.expander("View Errors", expanded=True):
                for err in res['errors']:
                    st.error(f"{err['json_file']}: {err['message']}")
...
# -----------------------------------------------------------------------------
# ASSEMBLER PAGE (SINGLE)
# -----------------------------------------------------------------------------

elif nav_mode == "Assembler":
    st.markdown("## Document Assembler (Single)")
...
# -----------------------------------------------------------------------------
    ...
    tabs = st.tabs(["Frontend", "Normalizers", "Engine", "Batch Engine"])
    ...
    with tabs[3]:
        st.markdown("#### src/batch_engine.py")
        try:
            engine_path = Path(__file__).parent.parent / "src" / "batch_engine.py"
            with open(engine_path, "r", encoding="utf-8") as f:
                st.code(f.read(), language="python", line_numbers=True)
        except Exception as e:
            st.error(f"Error: {e}")
