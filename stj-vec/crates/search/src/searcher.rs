...
use qdrant_client::qdrant::{value, SearchPointsBuilder, SparseIndices};
...
impl QdrantSearcher {
    ...
    pub async fn new(
        ...
        anyhow::ensure!(exists, "colecao '{collection}' nao encontrada no Qdrant");
    ...
    pub async fn search(
        ...
        let dense_request =
            SearchPointsBuilder::new(&self.collection, dense_vec.to_vec(), self.dense_top_k)
                .vector_name("dense")
                .with_payload(true);
        ...
        let sparse_request = SearchPointsBuilder::new(&self.collection, values, self.sparse_top_k)
            .vector_name("sparse")
        ...
        let dense_response =
            dense_result.map_err(|e| anyhow::anyhow!("busca densa falhou: {e}"))?;
        let sparse_response =
            sparse_result.map_err(|e| anyhow::anyhow!("busca esparsa falhou: {e}"))?;
...
fn extract_hits(points: &[qdrant_client::qdrant::ScoredPoint]) -> Vec<(String, f32)> {
    points
        .iter()
        .filter_map(|p| {
...
mod tests {
    ...
    fn test_rrf_single_side_only() {
        let dense = vec![("x".to_string(), 0.95), ("y".to_string(), 0.8)];
    ...
    fn test_rrf_respects_limit() {
        ...
        let sparse = vec![("d".to_string(), 15.0), ("e".to_string(), 12.0)];
    ...
    fn test_rrf_weighted_dense_bias() {
        // a: dense_rank=1, sparse_rank=2 -> rrf = 1/61 + 1/62 = 0.032522
        // b: dense_rank=2, sparse_rank=1 -> rrf = 1/62 + 1/61 = 0.032522
        // Com pesos iguais, empatam (ordem nao deterministica)
        let dense = vec![("a".to_string(), 0.9), ("b".to_string(), 0.85)];
        let sparse = vec![("b".to_string(), 15.0), ("a".to_string(), 10.0)];

        let equal = fuse_rrf(&dense, &sparse, 10, 60.0, 1.0, 1.0);
        // Ambos devem estar no resultado com scores iguais
        assert_eq!(equal.len(), 2);
        let a_score = equal.iter().find(|r| r.chunk_id == "a").unwrap().rrf_score;
        let b_score = equal.iter().find(|r| r.chunk_id == "b").unwrap().rrf_score;
        assert!((a_score - b_score).abs() < 1e-10, "a e b devem empatar com pesos iguais");

        // Dense bias 2.0: "a" vence (dense_rank=1 vale mais)
        // a: 2/61 + 1/62 = 0.03279 + 0.01613 = 0.04892
    ...
    async fn test_qdrant_search_integration() {
        let searcher = QdrantSearcher::new("http://localhost:6334", "stj", 10, 10, 60.0, 1.0, 1.0)
            .await
