...
const ThinkingBlock = ({ content, isStreaming, label = "Reasoning", duration }) => {
...
export default function CCuiChatInterface() {
  ...
  useEffect(() => {
    ...
    if (lastMessage.type === 'token' || lastMessage.type === 'chunk') {
        setMessages(prev => {
            const lastMsg = prev[prev.length - 1];
            if (lastMsg && lastMsg.role === 'assistant' && lastMsg.isStreaming) {
                return [
                    ...prev.slice(0, -1),
                    { ...lastMsg, content: lastMsg.content + (lastMessage.content || '') }
                ];
            } else {
                return [...prev, {
        ...
        setIsTyping(true);
    } else if (lastMessage.type === 'done' || lastMessage.type === 'end') {
        setIsTyping(false);
        setMessages(prev => {
            const lastMsg = prev[prev.length - 1];
  ...
  const handleSlashCommand = (cmd, args) => {
  ...
  const handleSendMessage = async () => {
    ...
    const userMessage = { id: Date.now().toString(), role: 'user', content: trimmedInput };
        ...
        `}>
          ...
          <button
            ...
            className={`transition-all duration-200 p-2.5 rounded-lg ${
              input.trim() === '' || isTyping ? 'text-[#333]' : 'text-[#d97757] hover:bg-[#d97757]/10 hover:scale-105'
            }`}
